...
class Post {
	...
	public function submitPost($body, $user_to, $imageName) {
		...
		$body = str_replace('\r\n', "\n", $body);
		$body = nl2br($body); 
      	$body= str_replace("Ỏ͖͈̞̩͎̻̫̫̜͉̠̫͕̭̭̫̫̹̗̹͈̼̠̖͍͚̥͈̮̼͕̠̤̯̻̥̬̗̼̳̤̳̬̪̹͚̞̼̠͕̼̠̦͚̫͔̯̹͉͉̘͎͕̼̣̝͙̱̟̹̩̟̳̦̭͉̮̖̭̣̣̞̙̗̜̺̭̻̥͚͙̝̦̲̱͉͖͉̰̦͎̫̣̼͎͍̠̮͓̹̹͉̤̰̗̙͕͇͔̱͕̭͈̳̗̭͔̘̖̺̮̜̠͖̘͓̳͕̟̠̱̫̤͓͔̘̰̲͙͍͇̙͎̣̼̗̖͙̯͉̠̟͈͍͕̪͓̝̩̦̖̹̼̠̘̮͚̟͉̺̜͍͓̯̳̱̻͕̣̳͉̻̭̭̱͍̪̩̭̺͕̺̼̥̪͖̦̟͎̻̰", "" , $body);
		...
		if($check_empty != "" || $imageName != "") {
			...
			if(strpos($no_punctuation, "height") === false && strpos($no_punctuation, "width") === false
				&& strpos($no_punctuation, "http") === false && strpos($no_punctuation, "youtube") === false && strpos($no_punctuation, "#") !== false){
				...
				foreach($stopWords as $value) {
					foreach($keywords as $key => $value2){
						if(strtolower($value) == strtolower($value2) || strpos($value2, "#") === false)
							$keywords[$key] = "";
			...
			while($row = mysqli_fetch_array($data_query)) {
				...
				foreach($body_array as $key => $value) {
						...
						if(strpos($value, "#") !== false) {
							...
							$value_tag= str_replace("#", "", $value);
							$body_array[$key] = '<a href="popular.php?tag='. $value_tag  . '">'. $value. '</a>';

						}
						if(strpos($value, "/>") !== false){
							$body_array[$key] = str_replace("/>", "", $value);
						}
				...
				if($user_logged_obj->isFriend($added_by)){
					...
					if($userLoggedIn == $added_by)
						$delete_button = "<a href='delete_post.php?id=$id' class='delete_button btn-danger' id='post$id'>X</a>";
					else 
						$delete_button = "";

					if($userLoggedIn == $added_by)
						$edit_button = "<a href='edit_post.php?id=$id'><button class='edit_button info'>Edit Post</button></a>";
					else 
						$edit_button = "";
					...
					$str .= "<div class='status_post column-posts' onClick='javascript:toggle$id(event)'>
								<div class='post_profile_pic'>
									<a href='$added_by'><img src='$profile_pic' width='50'></a>
								</div>
								...
								<div class='newsfeedPostOptions'>
									Comments($comments_check_num)&nbsp;&nbsp;&nbsp;</span>
		...
		if(mysqli_num_rows($data_query) > 0) {
			...
			$row = mysqli_fetch_array($data_query); 
				...
				if($user_logged_obj->isFriend($added_by)){
					...
					if($userLoggedIn == $added_by)
						$delete_button = "<a href='delete_post?id=$id.php' class='delete_button btn-danger' id='post$id'>X</a>";
					else 
						$delete_button = "";
	...
	public function loadPopularTags($data, $limit) {
	...
	public function getSinglePostEdit($post_id) {
			...
			$row = mysqli_fetch_array($data_query); 
				...
				if($user_logged_obj->isFriend($added_by)){
					...
					if($userLoggedIn == $added_by)
						...
						$delete_button = "";
					if($userLoggedIn == $added_by)
						$edit_button = "<button class='delete_button btn-danger' id='post$id'>X</button>";
					else 
						$delete_button = "";
					...
					$str .= "<div class='status_post column-posts' onClick='javascript:toggle$id(event)'>
								...
								<br>
								<br>
									<form class='post_form' name='edit_post' method='POST' action='edit_post.php?id=$id'>
									<textarea name='edit_text' id='post_text' >$body</textarea>
									<input type='submit' name='post' id='post_button' value='Edit'>
	...
	public function updatePost($body, $post_id) {
		$query = mysqli_query ($this->con, "UPDATE posts SET body='$body' WHERE id='$post_id'");

	}


	public function getSinglePostDelete($post_id) {
